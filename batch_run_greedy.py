"""
Simplified batch startup script for Greedy Search ablation.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Simplified: fix seed, run gated_mcts/run_greedy.py runs times asynchronously")
    ...
    parser.add_argument("--seed", type=int, required=True, help="Fixed seed (used for script and run_greedy.py)")
    ...
    parser.add_argument("--disable_qed_sa_gate", action="store_true")
    parser.add_argument("--use_geam_score", action="store_true")
...
def main():
    ...
    output_dir = Path(args.base_output_dir) / args.protein / f"seed{args.seed}"
    ...
    base_cmd = [
        ...
        "--min_terminals",
        str(args.min_terminals),
        "--max_split_depth",
        str(args.max_split_depth),
        "--init_children",
        str(args.init_children),
        "--n_total_children",
        str(args.n_total_children),
        "--c_param",
        str(args.c_param),
        "--width_increase_factor",
        str(args.width_increase_factor),
        "--add_value_weight",
        str(args.add_value_weight),
        "--n_simulations",
        str(args.n_simulations),
        "--fastrollout_weight",
        str(args.fastrollout_weight),
        "--max_n_repeat",
        str(args.max_n_repeat),
        "--diversity_threshold",
        str(args.diversity_threshold),
        "--max_resample_on_empty",
        str(args.max_resample_on_empty),
    ]
    ...
    if args.trace_path is not None and len(str(args.trace_path)) > 0:
        base_cmd += ["--trace_path", str(args.trace_path)]

    for i in range(args.runs):
        rand_tag = uuid.uuid4().hex[:8]
        out_name = f"greedy_job_{i}_seed_{args.seed}_sample_num_{args.sample_num}_{rand_tag}.csv"
        cmd = base_cmd + [
            "--device",
            str(args.device),
            "--seed",
