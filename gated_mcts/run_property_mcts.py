...
from gated_mcts.utils.local_property_oracle import LOCAL_PROPERTY_ORACLES, LocalPropertyOracle
...
def _save_history_csv(path: Path, predictor: LocalPropertyOracle) -> None:
...
def _save_topk_csv(path: Path, predictor: LocalPropertyOracle, k: int) -> None:
...
def _save_standard_topk_outputs(output_dir: Path, prefix: str, predictor: LocalPropertyOracle, custom_k: int) -> dict:
...
def _parse_args():
    ...
    parser.add_argument("--output_dir", type=str, default="results/property/softmol_mcts", help="Output directory")
    ...
    parser.add_argument("--oracle_name", type=str, default="qed", choices=list(LOCAL_PROPERTY_ORACLES), help="Local property oracle task name")
...
def main():
    ...
    print(f"[PROPERTY] history: {result['history_path']}")
    print(f"[PROPERTY] top10: {result['top10_path']}")
    print(f"[PROPERTY] top100: {result['top100_path']}")
    print(f"[PROPERTY] topk: {result['topk_path']}")
    print(f"[PROPERTY] metrics: {result['metrics_path']}")
    print(
        "[PROPERTY] n_calls={n_calls} auc_top10={auc_top10:.6f} top10={top10:.6f}".format(
