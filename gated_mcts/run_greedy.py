...
def _run_single(model, tokenizer, device, args, seed: int) -> tuple[float, float | None, str | None, str | None]:
    ...
    searcher = GreedySearcher(initial_state, config)
    t0 = time.time()
    with torch.no_grad():
...
def _parse_args():
    ...
    parser.add_argument("--output_file_name", default="greedy.csv", help="Output filename (with extension)")
    ...
    parser.add_argument("--min_terminals", type=int, default=-1, help="Compatibility only; unused in greedy baseline")
    parser.add_argument("--max_split_depth", type=int, default=100, help="Compatibility only; unused in greedy baseline")
    parser.add_argument("--init_children", type=int, default=20, help="Base candidate width for root step")
    parser.add_argument("--n_total_children", type=int, default=8, help="Base candidate width for non-root steps")
    parser.add_argument("--c_param", type=float, default=2.1, help="Compatibility only; unused in greedy baseline")
    parser.add_argument("--width_increase_factor", type=int, default=2, help="Compatibility only; unused in greedy baseline")
    ...
    parser.add_argument("--greedy_path", action="store_true", help="Compatibility only; unused in greedy baseline")
