"""
Simplified batch startup script for Beam Search ablation.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Simplified: fix seed, run gated_mcts/run_beam.py runs times asynchronously")
    ...
    parser.add_argument("--seed", type=int, required=True, help="Fixed seed (used for script and run_beam.py)")
...
def main():
    ...
    for i in range(args.runs):
        ...
        out_name = f"beam_job_{i}_seed_{args.seed}_sample_num_{args.sample_num}_{rand_tag}.csv"
        cmd = base_cmd + [
            "--device",
            str(args.device),
            "--seed",
