...
# Compatible with old checkpoints: historical module name my_tokenizer -> tool.my_tokenizer
import importlib
import sys
sys.modules.setdefault("my_tokenizer", importlib.import_module("tool.my_tokenizer"))
...
class Diffusion(L.LightningModule):
  def __init__(
    ...
    self.ignore_bos = self.config.algo.ignore_bos  # True, protects and ignores [BOS] token
    ...
    if self.config.algo.backbone == 'dit':
      ...
      try:
        ...
        raise ImportError('backbone=dimamba requires models/dimamba.py (currently not found in the repository or failed to import)') from e
  ...
  def on_validation_model_zero_grad(self) -> None:
    '''
    Note: previously used a hack to skip the first validation after resume by
    toggling `trainer.sanity_checking`. This interferes with Lightning's
    TQDMProgressBar initialization and can cause
    `TypeError: TQDMProgressBar._val_progress_bar reference has not been set`.
    We keep this hook as a no-op to avoid that issue.
    '''
  ...
  def _subs_parameterization(self, logits, xt):
    ...
    logits_logprob = logits - torch.logsumexp(logits, dim=-1, keepdim=True)
    unmasked = (xt != self.mask_index)
    onehot_logprob = torch.full_like(logits_logprob, self.neg_infinity)
    onehot_logprob.scatter_(-1, xt.unsqueeze(-1), 0.0)
    logits_final = torch.where(unmasked.unsqueeze(-1), onehot_logprob, logits_logprob)
    return logits_final
  ...
  def _nucleus_sample(self, p_x0):
    ...
    # Apply temperature if available (default 1.0)
    temp = getattr(self.config.sampling, 'temperature', 1.0)
    if abs(temp - 1.0) > 1e-6:
        # p_x0 is probability, convert to logits -> scale -> softmax
        logits = torch.log(p_x0 + 1e-10)
        p_x0 = F.softmax(logits / temp, dim=-1)
  ...
  def _ddpm_caching_update(self, x, t, dt, p_x0=None):
    ...
    # Control whether to enable top-1 position update via config (currently only effective in first_hitting path)
    use_top1 = bool(getattr(self.config.sampling, 'top1', True))

    if self.config.sampling.first_hitting:
      # Per-sample first hitting: update only one masked position per block for each sample
      # 1) Sample candidate tokens from model distribution for the whole block
      x_block = _sample_categorical(p_x0)
      masked = (x[:, -self.block_size:] == self.mask_index)  # (B, S_block)
      if use_top1:
        # Use top-1: select best index among masked positions based on confidence (max class prob)
        conf = p_x0.max(dim=-1).values  # (B, S_block)
        conf_masked = torch.where(
          masked,
          conf,
          torch.full_like(conf, float('-inf')),
        )
        ind = conf_masked.argmax(dim=-1)  # (B,)
        # If some samples have no updateable positions (all unmasked), fallback to last position (no value change)
        no_mask_any = ~masked.any(dim=1)
        ind = torch.where(
          no_mask_any,
          torch.full_like(ind, self.block_size - 1),
          ind,
        )
      else:
        # Don't use top-1: uniformly sample an index from masked positions
        probs = masked.float()
        probs_sum = probs.sum(dim=1, keepdim=True)  # (B,1)
        # Fallback if no masked positions (high fault tolerance)
        fallback = torch.zeros_like(probs)
        if fallback.shape[1] > 0:
          fallback[:, -1] = 1.0
        probs = torch.where(
          probs_sum > 0,
          probs / torch.clamp_min(probs_sum, 1.0),
          fallback,
        )
        ind = torch.multinomial(probs, 1).squeeze(1)  # (B,)

      # 2) Only update the selected index, keep others unchanged
      mask = (torch.arange(
        self.block_size, device=x.device) == ind[:, None]).to(x_block.dtype)
      x_block = x_block * mask + x[:, -self.block_size:] * (1 - mask)
    else:
      # Non-first-hitting path: keep original DDPM full-block random sampling logic;
      # To avoid generation quality degradation, ignore top-1 switch here (even if sampling.top1=True).
    ...
    with torch.amp.autocast('cuda', dtype=torch.float32):
      ...
      for i in range(num_pred_tokens):
        ...
        if self.config.sampling.var_length:
          ...
          # heuristic stop only after sufficient context
          if (i+1) > 256:
            stop, x_out = self._check_stop_conds(x[:, :i+1])
            if stop:
              x = x_out
  ...
  def _sample(
    ...
    if self.parameterization == 'ar':
      if self.config.sampling.var_length:
        ...
        for _ in range(self.config.sampling.num_sample_batches):
          ...
          text_samples.extend(self.tokenizer.batch_decode(sample_i))
        return text_samples
      else:
        for _ in range(self.config.sampling.num_sample_batches):
          sample_i, num_tries = None, 0
    ...
    if self.sampler == 'semi_ar':
      if self.config.sampling.var_length:
        # When sampling.var_length=true, decode directly to string per batch and append to list, avoiding torch.cat tensors.
        text_samples = []
        for _ in range(self.config.sampling.num_sample_batches):
          sample_i, num_tries = None, 0
          while sample_i is None:
            ...
            if num_tries > 10:
              raise ValueError('Sampling failed.')
          text_samples.extend(self.tokenizer.batch_decode(sample_i))
          # Disable metrics stats during sampling (SMILES uses text eval only)
        return text_samples
      else:
        for _ in range(self.config.sampling.num_sample_batches):
          sample_i, num_tries = None, 0
  ...
  def _semi_ar_sampler(
    ...
    # Read an optional prefix (string or per-sample list) for prefix-conditioned sampling.
    prefix_text = getattr(self.config.sampling, 'prefix', None)
    # OmegaConf ListConfig compatibility.
    if prefix_text is not None and not isinstance(prefix_text, (str, list)):
        try:
            prefix_text = list(prefix_text)
        except:
            pass

    next_block_only = bool(getattr(self.config.sampling, 'next_block_only', False))

    # Build prefix tensor (B, L): non-prefix positions are mask_index; prefix is
    # [BOS] + encode(prefix)[:seqlen-1].
    prefix_tensor = None
    prefix_lens = None
    if prefix_text is not None:
      prefix_list = None
      if isinstance(prefix_text, str) and len(prefix_text) > 0:
        prefix_list = [prefix_text] * n_samples
      elif isinstance(prefix_text, list):
        if len(prefix_text) != n_samples:
          raise ValueError(f"prefix list length {len(prefix_text)} must match n_samples {n_samples}")
      ...
      if prefix_list is not None:
        ...
        reserved = {eos_id, pad_id, self.mask_index}

        prefix_tensor = torch.full((n_samples, seqlen), self.mask_index, dtype=torch.long, device=self.device)
        prefix_lens = torch.zeros(n_samples, dtype=torch.long, device=self.device)
        max_body = max(0, seqlen - 1)
        for b in range(n_samples):
          p = prefix_list[b] or ''
          ids = self.tokenizer.encode(p, add_special_tokens=False)
          # Drop reserved tokens to avoid EOS/PAD/MASK interfering with sampling.
          ids = [tid for tid in ids if tid not in reserved]
          ids = ids[:max_body]
          toks = [bos_id] + ids
          Lb = len(toks)
          prefix_lens[b] = Lb
          prefix_tensor[b, :Lb] = torch.tensor(toks, dtype=torch.long, device=self.device)
    
    # Reset KV cache.
    if self.config.sampling.kv_cache:
      # DIT.reset_kv_cache() and HF implementations both accept no args.
      self.backbone.reset_kv_cache()

    # next_block_only: assertions and target block index (BOS-aligned).
    target_stride = 0
    if next_block_only:
      assert self.config.sampling.first_hitting, 'next_block_only requires sampling.first_hitting=True'
      assert num_steps >= self.block_size, 'next_block_only requires num_steps >= block_size'
      assert not mdlm_semi_ar, 'next_block_only is not supported in mdlm mode'
      if prefix_lens is not None:
        prefix_total = int(prefix_lens[0].item())
        assert (prefix_total % self.block_size) == 0, 'requires (1 + len(encode(prefix))) % block_size == 0'
        assert prefix_total + self.block_size <= seqlen, 'prefix too long: no next block to generate'
        target_stride = prefix_total // self.block_size
      else:
        target_stride = 0
    ...
    for stride_num in range(num_strides):
      ...
      # Before intra-block diffusion, copy prefix tokens into x_accum for this block;
      # _ddpm_caching_update(copy_flag) will protect them from being overwritten.
      if prefix_tensor is not None:
        if len(fwd_idx) > 0:
          blk_start = int(fwd_idx[0].item())
          blk_end = int(fwd_idx[-1].item()) + 1
        else:
          blk_end = (stride_num + 1) * self.block_size
          blk_start = max(0, blk_end - self.block_size)
        for b in range(n_samples):
          Lb = int(prefix_lens[b].item())
          if Lb > blk_start:
            end = Lb if Lb < blk_end else blk_end
            if end > blk_start:
              x_accum[b, blk_start:end] = prefix_tensor[b, blk_start:end]

      dt = 1 / num_steps
      p_x0_cache = None
      timesteps = torch.linspace(1, 0, num_steps, device=self.device)
      t = torch.ones(n_samples, dtype=self.dtype, device=self.device)
      for i in range(num_steps):
        ...
        if self.config.sampling.first_hitting:
          ...
          num_masked = (x_accum[:, fwd_idx] == self.mask_index).sum(-1)
          num_masked = num_masked.clamp_min(1).to(torch.float32)
          t = t * torch.pow(u, 1.0 / num_masked)
        ...
        if self.config.sampling.first_hitting:
          ...
          t_in = (t * torch.ones((n_samples, 1), dtype=self.dtype, device=self.device))
      ...
      # next_block_only: return right after the target block (no EOS appended).
      if next_block_only and stride_num >= target_stride:
        out_len = (stride_num + 1) * self.block_size
        return x_accum[:, :out_len], sampling_steps

      # Stop checks are disabled until the target block is generated in next_block_only mode.
      if self.config.sampling.var_length and (not next_block_only or stride_num >= target_stride):
        stop, x_accum = self._check_stop_conds(x_accum)
        if stop:
          break
      else:
        # heuristic stop only after sufficient context
        if x_accum.shape[1] > 256 and (not next_block_only or stride_num >= target_stride):
          stop, x_accum = self._check_stop_conds(x_accum)
          if (stop and not self.config.sampling.var_length) \
            or (stop and x.shape[-1] == 1):
            return None, None
  ...
  def _check_stop_conds(self, x):
    """Check whether to stop sampling and optionally freeze finished samples.

    For var_length=true:
    - Stop only when every sample in the batch has produced at least one EOS.
    - Once a sample hits EOS, fill all positions after its first EOS with EOS to freeze it.

    For var_length=false, keep the (optional) entropy-based early stop behavior.

    Args:
      x: torch.Tensor, current sample (B, L).
    Returns:
      stop: bool, whether to stop sampling.
      x: torch.Tensor, potentially modified (finished rows frozen after EOS).
    """
    stop = False

    # Optional entropy-based early stop (usually disabled for SMILES).
    stop_on_eos_only = getattr(self.config.sampling, 'stop_on_eos_only', False)
    use_entropy_stop = getattr(self.config.sampling, 'entropy_stop', True)
    entropy = None
    if (not stop_on_eos_only) and use_entropy_stop:
      entropy = self._compute_entropy(x[:, -256:])
      if entropy < 4:
        stop = True

    if self.config.sampling.var_length:
      # Per-sample EOS detection; freeze everything after the first EOS.
      eos_id = self.tokenizer.eos_token_id
      eos_mask = (x == eos_id)
      has_eos = eos_mask.any(dim=1)  # (B,)

      if has_eos.any():
        # First EOS position per sample (-1 for samples without EOS).
        first_pos = eos_mask.float().argmax(dim=1)  # all-False rows return 0
        minus_ones = torch.full_like(first_pos, -1)
        first_pos = torch.where(has_eos, first_pos, minus_ones)  # (B,)

        # Fill positions after the first EOS with EOS (only for finished samples).
        pos_idx = torch.arange(x.shape[1], device=x.device)[None, :]  # (1, L)
        after_eos = pos_idx > first_pos[:, None]                      # (B, L)
        freeze_mask = after_eos & has_eos[:, None]
        if freeze_mask.any():
          x = x.masked_fill(freeze_mask, eos_id)

        # Stop only when all samples have hit EOS.
        if has_eos.all():
          stop = True

    else:
      # Fixed-length: keep optional entropy stop; no extra handling for stop_on_eos_only.
      pass
